...
topFive2022 = topFive2023 = topFive2024 = topFive2025 = 0
wins2022 = wins2023 = wins2024 = wins2025 = 0
...
if not driverData.empty:
    ...
    data2022 = driverData[driverData['Track/Year'].str.contains('2022', na=False)].copy()
    print(f"\n{driver}'s finishes in 2022:")#Display previous race results
    print(data2022[['Pos', 'St', 'Track/Year']].reset_index(drop=True).to_string(index=False))
    position2022 = data2022['Pos'].values#keep track of data in the Pos column in excel file
#Check for fives and wins
    for pos in position2022:
        if pos <= 5:
            topFive2022 += 1
            if pos == 1:
                wins2022 += 1
...
print(f"\nTop fives for {driver} in 2022: {topFive2022}")
print(f"Wins for {driver} in 2022: {wins2022}")
...
top_fives = [topFive2022, topFive2023, topFive2024, topFive2025]
wins = [wins2022, wins2023, wins2024, wins2025]
...
for df, year in [(data2022, 2022), (data2023, 2023), (data2024, 2024), (data2025, 2025)]:
...
full_data = pd.concat([data2022, data2023, data2024, data2025], ignore_index=True)
